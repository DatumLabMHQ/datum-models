...
-- summing a market's rows on one side says how much of it the sample covers. Incremental like the
-- market mart: raw rows older than 60 days tier to R2, so only the last 3 days are recomputed each run.
with snaps as (
...
select s.day, s.fetched_at as as_of, s.chain_id, s.market_id, s.side, s.rank, s.user_address,
       ...
       case when s.side = 'supply' and s.market_supply_usd > 0 then s.supply_assets_usd / s.market_supply_usd * 100
