...
-- reported health factor count in collateral_tracked_usd but in no band.
with p as (
...
select base.day, base.as_of, base.chain_id, base.market_id, base.market_supply_usd, base.market_borrow_usd,
       ...
       case when base.market_borrow_usd > 0 then coalesce(b.borrow_tracked_usd, 0) / base.market_borrow_usd * 100 end as borrow_coverage_pct,
       ...
       case when base.market_supply_usd > 0 then coalesce(s.supply_tracked_usd, 0) / base.market_supply_usd * 100 end as supply_coverage_pct,
       case when base.market_supply_usd > 0 then coalesce(s.top5_supply_usd, 0) / base.market_supply_usd * 100 end as top5_supply_share_pct
from base
